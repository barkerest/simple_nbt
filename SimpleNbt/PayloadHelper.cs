...
namespace SimpleNbt
{
	internal static class PayloadHelper
	{
		public static readonly (TagType, TagType, Func<Stream, string, INamedBinaryTag>)[] Builders =
		{
			...
			(TagType.List, TagType.Byte, (s,      nm) => new ByteListTag(nm)),
			(TagType.List, TagType.Short, (s,     nm) => new ShortListTag(nm)),
			(TagType.List, TagType.Int, (s,       nm) => new IntListTag(nm)),
			(TagType.List, TagType.Long, (s,      nm) => new LongListTag(nm)),
			(TagType.List, TagType.Float, (s,     nm) => new SingleListTag(nm)),
			(TagType.List, TagType.Double, (s,    nm) => new DoubleListTag(nm)),
			(TagType.List, TagType.String, (s,    nm) => new StringListTag(nm)),
			(TagType.List, TagType.ByteArray, (s, nm) => new ByteArrayListTag(nm)),
			(TagType.List, TagType.Compound, (s,  nm) => new CompoundListTag(nm)),
		...
		public static INamedBinaryTag DecodeTag(this Stream input)
		{
			...
			var type = (TagType) b;
			if (type == TagType.End) return EndTag.Instance;
			...
			else
			{
				...
				if (builders.Length == 0) throw new InvalidDataException($"Failed to locate builder for {type}:{subtype} tag.");
				if (builders.Length != 1) throw new InvalidOperationException($"Located multiple builders for {type}:{subtype} tag.");
				var child = builders[0].Item3(input, name);
				child.DecodePayload(input);
				return child;
			}
